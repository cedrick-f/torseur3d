...
class Variable:
    ...
    def Augmenter(self, coef = 1, rnd = None):
#        print "Augmenter", coef, self.v[0]
...
#        self.Bind(wx.EVT_CHAR, self.OnChar, self.text)
        ...
        vs.Add( txtnom, 0, wx.ALIGN_CENTRE_VERTICAL|wx.ALIGN_RIGHT|wx.LEFT, 0 )
        vs.Add(self.text, 1, wx.ALIGN_CENTRE|wx.LEFT|wx.RIGHT, 2 )
        vs.Add(self.spin, 0, wx.ALIGN_CENTRE|wx.LEFT|wx.RIGHT, 0 )
...
#            return
#        
#        print "OnScroll", self.lastPos, ">", event.GetPosition(), "(", self.variable.v[0], ")"
        
        pos = event.GetPosition()
        if pos != self.lastPos:
            
            # Quand on commence le Scroll, on "désactive"
            if self.etatInit == None and self.fct != None:
                self.etatInit = self.fct() # Etat d'activation initial
                self.fct(False) # Désactivation
            
            # Effet du Scroll ...
            coef = self.coef * (pos-self.lastPos)
            self.variable.Augmenter(coef, rnd = 1)
#            if pos > self.lastPos:
#                
#            else:
#                self.variable.Diminuer(coef)
...
#        print "mofifierValeurs", self.variable.n
